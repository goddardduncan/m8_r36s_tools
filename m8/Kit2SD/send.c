...
const char *menu_items[] = {"MOUNT USB", "SEND KIT TO SD/SAMPLES/SLICED", "EXIT"};
...
// Function to transfer drum kits
void send_drums_to_sd() {
    log_message("[INFO] Starting file transfer...");
    
    system("sudo mkdir -p /USB/samples/Sliced");  // Ensure destination exists
    system("sudo chmod 777 /USB/samples/Sliced"); // Ensure it's writable

    if (access("/USB/samples/Sliced", W_OK) != 0) {
        log_message("[ERROR] /USB/samples/Sliced is not writable!");
        return;
    }

    DIR *dir = opendir("/roms/drums/sliced");
    if (!dir) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        snprintf(src_path, sizeof(src_path), "/roms/drums/sliced/%s", entry->d_name);
        snprintf(dest_path, sizeof(dest_path), "/USB/samples/Sliced/%s", entry->d_name);
