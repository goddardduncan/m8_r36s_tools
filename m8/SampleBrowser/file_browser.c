...
#define SERVER_URL "http://151.101.82.59/upload"
#define SAVED_FOLDER "/roms/drums/saved/"
#define OUTPUT_FILENAME "GAEMBOY.wav"
...
// Function to upload all .wav files in /roms/drums/saved/
void upload_saved_wavs() {
    DIR *dir = opendir(SAVED_FOLDER);
    if (!dir) {
    ...
    struct curl_slist *headerlist = NULL;  // For setting additional headers
    ...
    // ✅ Add a custom Host header in case the server needs it
    headerlist = curl_slist_append(headerlist, "Expect:");

    // ✅ Set the filename field
    curl_formadd(&formpost, &lastptr,
                 CURLFORM_COPYNAME, "filename",
                 CURLFORM_COPYCONTENTS, OUTPUT_FILENAME,
                 CURLFORM_END);

    // ✅ Loop through files in /roms/drums/saved/
    while ((entry = readdir(dir)) != NULL && file_count < 32) {
        if (entry->d_type == DT_REG && strstr(entry->d_name, ".wav")) { 
            char file_path[MAX_PATH_LEN];
            snprintf(file_path, sizeof(file_path), "%s/%s", SAVED_FOLDER, entry->d_name);

            printf("📤 Adding file to upload: %s\n", file_path);
    ...
    if (file_count == 0) {
        printf("❌ No WAV files found to upload.\n");
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return;
    }

    printf("📤 Uploading %d WAV files to merge as %s...\n", file_count, OUTPUT_FILENAME);
    ...
    if (res == CURLE_OK) {
        printf("✅ Successfully uploaded and requested merge: %s\n", OUTPUT_FILENAME);
    } else {
...
void load_directory(const char *path);  // Add this before enter_directory()
...
file_count = 0;
    ...
    // If in /roms/drums/samples/, add "DELETE CURRENT KIT" as the first item
    if (strcmp(path, base_path) == 0) {
        file_list[file_count++] = strdup("DELETE CURRENT KIT");
    }

    // If not in the base path, add ".." to go up
    if (strcmp(path, base_path) != 0) {
...
void delete_saved_kit() {
    DIR *dir = opendir(saved_path);
    if (!dir) {
        log_message("ERROR: Unable to access saved directory.");
        return;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG && strstr(entry->d_name, ".wav")) {
            snprintf(file_path, sizeof(file_path), "%s/%s", saved_path, entry->d_name);
    ...
    closedir(dir);
    log_message("Drum kit deleted successfully.");
    load_directory(saved_path);
}
...
void run_sdl_ui() {
    ...
    if (strcmp(file_list[selected_index], "DELETE CURRENT KIT") == 0) {
        delete_saved_kit();
    } else if (strcmp(file_list[selected_index], "...") == 0) {
    ...
    break;  // ⚠️ Ensure the break is here
